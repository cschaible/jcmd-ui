<script>
	...
	function threadInformation(t) {
		if (t !== undefined) {
			if (t.applicationThreads !== undefined) {
				tableData = prepareTableData(t.applicationThreads);
			}
			if (t.threadCountApplication !== undefined && t.threadCountApplication.values !== undefined) {
				chartData = threadChart(t.threadCountApplication.values);
			}
		}
	}
	...
	function prepareTableData(values) {
		if (values !== undefined) {
			...
			return values.sort(
				(a, b) =>
					a.status.localeCompare(b.status) || a.name.localeCompare(b.name) || b.elapsed - a.elapsed
			);
		}
		return undefined;
	}
	...
	function threadChart(values) {
		if (values !== undefined) {
			...
			for (const v of values) {
				...
				totalCountValues.push(
					v.newCount + v.runnableCount + v.waitingCount + v.timedWaitingCount + v.blockedCount
				);
			}
			...
			let newDataset = newDataSet(
				...
				newCountAvg,
				newCountMedian,
				'rgba(212,12,243,0.75)',
				'rgba(212,12,243,0.1)',
				'+1'
			);
			let runnableDataset = newDataSet(
				'Runnable',
				runnableCountValues,
				runnableCountMin,
				runnableCountMax,
				runnableCountAvg,
				runnableCountMedian,
				'rgba(158,243,12,0.75)',
				'rgba(158,243,12,0.1)',
				true
			);
			let waitingDataset = newDataSet(
				'Waiting',
				waitingCountValues,
				waitingCountMin,
				waitingCountMax,
				waitingCountAvg,
				waitingCountMedian,
				'rgba(12,162,243,0.75)',
				'rgba(12,162,243,0.1)',
				true
			);
			let timedWaitingDataset = newDataSet(
				'Timed Waiting',
				timedWaitingCountValues,
				timedWaitingCountMin,
				timedWaitingCountMax,
				timedWaitingCountAvg,
				timedWaitingCountMedian,
				'rgba(243,224,12,0.75)',
				'rgba(243,224,12,0.1)',
				true
			);
			let blockedDataset = newDataSet(
				'Blocked',
				blockedCountValues,
				blockedCountMin,
				blockedCountMax,
...
<div class="thread-dashboard">
	<ButtonGroup class="thread-visualization-selection">
		<Button outline color="primary" active={chartActive} on:click={toggleVisualization}>
			Chart
		</Button>
		<Button outline color="primary" active={tableActive} on:click={toggleVisualization}>
			Table
		</Button>
	</ButtonGroup>
