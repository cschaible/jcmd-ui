...
impl JvmThreadTab {
    pub fn new(title: String) -> JvmThreadTab {
        let (tx, rv) = channel();
    ...
    fn draw_table(&mut self, f: &mut Frame, area: Rect, metrics: &Threads) {
        self.state_length = metrics.jvm_threads.len();
        self.state = self
            .state
            .content_length(metrics.jvm_threads.len() - 1)
        ...
        let mut sorted_metrics = metrics.jvm_threads.clone();
    ...
    fn draw(&mut self, f: &mut Frame, area: Rect) {
        ...
        if let Some(metrics) = self.metrics.clone() {
            ...
            if self.chart_visible {
                ...
                let chart_data = get_thread_metrics(&metrics.thread_count_jvm, width);
